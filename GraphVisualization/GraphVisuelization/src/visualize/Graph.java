...
public class Graph {
	...
	private Pane nd,ed,ndLab,edLab; //Panes for nodes and edges and their labels
	...
	public void addNode(String id) throws ExistException { // adding Node
		...
		if(nd!=null) { 
			...
			nd.getChildren().add(nodes.get(id)); 
			ndLab.getChildren().add(nodes.get(id).getLabel());
	...
	public void addEdge(String id1,String id2,String l) throws ExistException { // adding Edge
		...
		if(ed!=null) { 
			...
			ed.getChildren().add(e); // adding edge to javafx Pane
			edLab.getChildren().add(edges.get(p).getLabel()); // add to edges label pane
	...
	public Edge removeEdge(String id1,String id2) throws NotExistException { // remove Edge returns removed Edge or null if it's empty
		...
		if(ed!=null) { 
			...
			ed.getChildren().remove(edges.get(p));
			edLab.getChildren().remove(edges.get(p).getLabel());
	...
	public Node removeNode(String id) throws NotExistException { // remove Node returns deleted Node or null if it's empty
		...
		if(nd!=null) {
			...
			ndLab.getChildren().remove(nodes.get(id).getLabel());
			nd.getChildren().remove(nodes.get(id));
		}
	...
	public void setPaneNodesLabels(Pane nodesLab) { // setting Pan for nodes labels
		
		ndLab=nodesLab;
		
	}
	
	public void setPaneEdgesLabels(Pane edgesLab) {
	...
	public double getGraphCenterX() { // finding graph width and finding the point in the middle of it relative to 0,0
	...
	public double getGraphCenterY() { // finding graph height and finding the point in the middle of it relative to 0,0
	...
	public static void main(String[] args) {
		...
		g.getNode("6").setCenterX(4);
		g.getNode("7").setCenterX(5);
		g.getNode("6").setCenterY(1);
		g.getNode("7").setCenterY(5);
		System.out.println(g.getGraphCenterX());
		System.out.println(g.getGraphCenterY());
		/*g.addEdge("4","5","4-5");
		g.addEdge("4", "6", "4-6");
		g.addEdge("5", "6", "5-6");
...
public ArrayList<String> findAllPaths(String src,String dst){ // function for finding all paths between two nodes
